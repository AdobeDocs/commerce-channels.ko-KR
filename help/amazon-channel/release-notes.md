...
자세한 내용은 [예정된 릴리스](https://devdocs.magento.com/release/){target="_blank"} 버전 관리, 지원 및 호환성이 필요합니다.
...
![해결된 문제](../assets/fix.svg) 로그 노이즈 감소.

![해결된 문제](../assets/fix.svg) 업데이트 가져오기의 안정성이 개선되었습니다.

![해결된 문제](../assets/fix.svg) 단일 작업과 같은 풀 실행 또는 CLI에서 적용하는 프로세스를 단순화했습니다.

![해결된 문제](../assets/fix.svg) 에 대한 종속성을 업그레이드했습니다. `magento/services-connector`.

![해결된 문제](../assets/fix.svg) 잘못된 국가 코드가 있는 영국 계정의 동기화 문제가 수정되었습니다.

![해결된 문제](../assets/fix.svg) 카탈로그 제품 엔티티에 대해 entity_type_id를 하드코딩하면 Magento 가격 출처에 문제가 발생합니다.

![해결된 문제](../assets/fix.svg) 다른 인스턴스에서 백엔드에서 삭제된 계정이 UI에서 삭제되지 않는 문제를 수정했습니다.

![해결된 문제](../assets/fix.svg) 일부 장바구니 규칙이 주문 가져오기를 중단하는 문제가 해결되었습니다.
...
이전 버전이 있는 경우 [!DNL Amazon sales channel] 버전이 설치되어 있고 Adobe Commerce을 버전 2.4.0으로 업데이트하려고 하면 Adobe Commerce 업데이트를 완료하기 전에 확장을 업데이트하라는 메시지가 표시됩니다.
